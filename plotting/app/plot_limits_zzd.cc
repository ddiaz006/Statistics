...
int main( int argc, char** argv )
{
  ...
  */
  ...
  if( ifs.is_open() )
    ...
    int low = fname.find("Sig_h250_llp50_ct");
	  int high = fname.find("/higgsCombineTest");
	  std::string mass = fname.substr( low+17, high-(low+17) );
	  float _mass = atof( mass.c_str() );
	  std::cout << "mass: " << _mass << std::endl;
  ...
  */
  ...
  gTwoS->GetXaxis()->SetTitleSize(0.05);
  gTwoS->GetXaxis()->SetLabelOffset( 0.003);
  gTwoS->GetXaxis()->SetTitleOffset( 0.97);
  gTwoS->GetYaxis()->SetTitleSize(0.05);
  gTwoS->GetYaxis()->CenterTitle(kTRUE);
  gTwoS->GetYaxis()->SetTitleOffset( 0.90);
  gTwoS->GetXaxis()->SetTitle("s proper decay length [mm]");
  gTwoS->GetYaxis()->SetTitle("#sigma(pp#rightarrow#Phi)#timesB(#Phi#rightarrowZZ_{d}) [pb]");
  //gObs->GetXaxis()->SetTitle("s proper decay length [mm]");
  //gObs->GetYaxis()->SetTitle("#sigma(pp#arrow#Phi)#times B(#Phi#arrowZZ_{d}) [pb]");
